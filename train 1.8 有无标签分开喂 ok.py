...
class Train(object):
    ...
    def train(self):
        ...
        for epoch in range(self.EPOCH):
            ...
            iters = 100//self.batch_size
            flag2 = 1  # if epoch>10 else 0
            for idx in range(iters):
                start_t = time.time()
                flag = 1 if idx < 4 else 0 # set we use 2*batch_size=200 train data labeled.
                batchx, batchl = mnist.train.next_batch(self.batch_size,self.cw_x_train,self.cw_y_train,test=2)
...
# output = conv2d('Z_cona{}'.format(i), output, 3, 64, stride=1, padding='SAME')
    ...
    def test(self):
        count = 0.
        print ('testing................')
        for i in range(1000//self.batch_size):
            testx, textl = mnist.train.next_batch(self.batch_size,self.cw_x_test,self.cw_y_test,test=2)
