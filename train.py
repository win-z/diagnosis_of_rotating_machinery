...
class Train(object):
    ...
    def build_model(self):
        ...
        self.lx=tf.placeholder(tf.float32,shape=[self.batch_size,self.img_size*self.img_size*self.dim],name='lreal_img')
        ...
        self.llabel = tf.placeholder(tf.float32, shape=[self.batch_size, self.num_class - 1], name='llabel')
        ...
        d_logits_lr, layer_out_lr = self.discriminator('dis', self.lx, reuse=True)   #数据集经过判定器
        #####################################################？？？？？？？？？？？？？？？
        ...
        logits_lr = tf.nn.softmax(d_logits_lr)
        ...
        s_label = tf.concat([self.llabel, tf.zeros(shape=(self.batch_size,1))], axis=1)
        test_s_label = tf.concat([self.label, tf.zeros(shape=(self.batch_size,1))], axis=1)
        s_l_r = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=s_label*0.9, logits=d_logits_lr))
        ...
        self.prediction = tf.nn.in_top_k(test_logits, tf.argmax(test_s_label, axis=1), 1)
    ...
    def train(self):
        ...
        for epoch in range(self.EPOCH):
            ...
            for idx in range(iters):
                ...
                if (idx+1)*self.batch_size%self.ltrain_size != 0:
                    lbatchx, lbatchl = self.cw_x_train[((idx*self.batch_size)%self.ltrain_size):((idx+1)*self.batch_size%self.ltrain_size)],self.cw_y_train[((idx*self.batch_size)%self.ltrain_size):((idx+1)*self.batch_size%self.ltrain_size)]
                if (idx+1)*self.batch_size%self.ltrain_size == 0:    
                    lbatchx, lbatchl = self.cw_x_train[((idx*self.batch_size)%self.ltrain_size):self.ltrain_size],self.cw_y_train[((idx*self.batch_size)%self.ltrain_size):self.ltrain_size]
                ...
                feed = {self.x:batchx, self.z:noise, self.label:batchl, self.lx:lbatchx, self.llabel:lbatchl, self.flag:flag, self.flag2:flag2}
