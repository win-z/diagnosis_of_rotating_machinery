...
class Train(object):
    ...
    def build_model(self):
        ...
        self.d_loss = (d_loss_r - d_loss_f)*(1-self.flag) + s_l_r*self.flag*10 + d_regular
    ...
    def train(self):
        ...
        for epoch in range(self.EPOCH):
            ...
            for idx in range(iters):
                start_t = time.time()
                flag = 1 if idx < (self.real_train//self.batch_size) else 0 # set we use 2*batch_size=200 train data labeled.
...
# output = conv2d('Z_cona{}'.format(i), output, 3, 64, stride=1, padding='SAME')
    ...
    def test(self):
        ...
        for i in range(self.test_size//self.batch_size):
            testx, textl = mnist.train.next_batch(self.batch_size,self.cw2_x_test,self.cw2_y_test,test=2)
